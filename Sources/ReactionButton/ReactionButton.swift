...
open class ReactionButton: UIButton {
  ...
  private var selectedItem: Int? {
    didSet {
      if oldValue != selectedItem {
        delegate?.reactionButton(self, didChangeFocusTo: selectedItem)
      }
    }
  }
  ...
  private var config: ReactionButton.Config {
    ...
    return delegate.reactionButton(configurationFor: self)
  }

  private var rootView: UIView? {
    context?.view
    ?? self.ownerViewController?.view
  ...
  public init(frame: CGRect, context: UIViewController) {
  ...
  private func expand() {
    ...
    for i in 0..<_dataSource.reactionButton(numberOfOptionsFor: self) {
      let optionFrame = CGRect(x: xPosition(for: i), y: config.heightForSize * 1.2,
                               sideSize: config.sizeBeforeOpen)
      let option = _dataSource.reactionButton(self, viewForIndex: i)
  ...
  private func move(_ point: CGPoint) {
    ...
    if optionsBarView.contains(point) {
      let relativeSizePerOption = optionsBarView.frame.width / CGFloat(_dataSource.reactionButton(numberOfOptionsFor: self))
  ...
  private func collapse() {
    for (index, option) in optionsBarView.subviews.enumerated() {
      UIView.animate(index: index) {
        ...
        guard finished, index == self._dataSource.reactionButton(numberOfOptionsFor: self)/2 else {
          return
        }
        self.optionsBarView.removeFromSuperview()
        self.optionsBarView.subviews.forEach { $0.removeFromSuperview() }
        if let selectedItem = self.selectedItem {
          self.delegate?.reactionButton(self, didSelectIndex: selectedItem)
        } else {
          self.delegate?.reactionButton(didCancelActionFor: self)
        }
      }
    }
  }
  ...
  private func focusOption(withIndex index: Int) {
    guard (0..<_dataSource.reactionButton(numberOfOptionsFor: self)).contains(index) else { return }
    selectedItem = index
    let config = self.config
  ...
  private func updateOptionsView(with trait: UITraitCollection) {
    ...
    optionsBarView.frame = config.rect(items: _dataSource.reactionButton(numberOfOptionsFor: self),
